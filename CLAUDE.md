...
Project-skills: **add-provider** (nieuwe partner-SDK bouwen + aan de Hub koppelen) · **change-sdk** (bestaande SDK wijzigen, met de tabel "raak ik de Hub aan?") · **unified-api-review** (architectuurreview van de Unified-API-kern door de provider-#2-lens; schrijft rapport, wijzigt geen code). Documentatie-drift: `/ai:docs-sync` (ai-kit, geen los project-skill), proactief na domein-wijzigingen en vóór commit. Authoritative conventies staan in `.agents/rules/` (auto-loaded: taal, engineering, security); ai-kit canonical rules in `.claude/rules/` (gitignored, aanvullend).
