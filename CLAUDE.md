...
- **Admin-paneel**: Filament v4 op `/admin` (Phase 9, HUB-04). `User` implementeert `FilamentUser` + `HasRoles` (Spatie); admin-access via Spatie-rollen `super-admin`/`staff`/`boekhouder` (zie `EmeqStaffSeeder`). Resource-management voor `manage-staff` ge-gate via gate in `AppServiceProvider::boot()`. Hub-resources gegroepeerd in 6 NL navigation-groups (Consumers / Integraties / Abonnementen / Facturatie / Boekhouding / Beheer), incl. de read-only `PassThroughCallResource` (Integraties, gate `view-pass-through-calls`). De **boekhouding** ("Books", ERPSAAS-gebaseerd) leeft top-level in ditzelfde paneel onder de groepen **Facturatie** (Facturen/Terugkerend/Inkoopfacturen/Klanten/Leveranciers) + **Boekhouding** (Grootboek/Transacties/Memoriaal/Overzichten) — géén aparte cluster meer, resources op `/admin/{invoices,bills,clients,…}`; functiescheiding per-resource via trait `App\Filament\Books\Concerns\GatedToBoekhouding` (super-admin/boekhouder zien de boekhoud-groepen, staff niet). Zie `.docs/decisions/erpsaas-books-module.md`.
