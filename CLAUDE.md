...
De hele stack draait lokaal in Docker (`docker compose up -d --build` → `http://hub.emeq.test:8092`, health-check `/up`). Dev = FrankenPHP `watch` + Vite-HMR, geen rebuild bij code-changes. Prod: `make prod-deploy` — health-gated zero-downtime swap van `app` via Traefik + `docker-rollout` (#109); een rauwe `docker compose up -d --build` recreate't in place en geeft downtime. Setup + doc-URLs: `docs/agents/dev-environment.md`. Services en `Dockerfile`-targets: `docs/agents/docker.md`. Deploy-mechanisme: `docs/deployment.md`. Lagen en componenten: `docs/agents/architecture.md`.
...
- **Idempotency** — `Idempotency-Key`-header via `EnsureIdempotency`; `idempotent:required` op accounting-documents. Tweede laag: `provider_entity_links` dedupliceert op `(connection, external_id)`, of op `(consumer, external_id)` bij een keyless provider, ook nadat de key weg is — en die is er wél weg: `idempotency_keys` wordt dagelijks geprund op 24 uur.
- **Observability** — één `request_id` (ULID) via `AssignRequestId`, in `Context` → logs, queued jobs, `pass_through_calls`, `inbound_webhook_events` en de `X-Emeq-Request-Id`-header op consumer-webhooks. Elke `/v1/*`-response logt één metadata-only regel `api.request` via `LogApiRequest` (methode, pad, query-keys, status, duur, consumer, body-fingerprint) — nooit headers of body.
...
- **iTheorie** — eerste provider **zonder Connection**: één Hub-eigen broker-inlog in `ItheorieSettings`, dus geen `X-Account-Id`. Zes routes op `/v1/itheorie/*`, géén catch-all: elke POST koopt een code van €9,70 en is onomkeerbaar. `PurchaseLedger` legt de Idempotency-Key duurzaam vast in `provider_entity_links` en bewaakt daarmee zowel dubbel-kopen als eigenaarschap; er is bewust geen lijst-endpoint, want de partner geeft altijd álle aankopen van de reseller terug. Zie `.docs/decisions/itheorie-keyless-provider.md`.
...
ai-kit draait als plugin (`.ai-kit-setup`: `tier=full`, `mode=project-only`). Lifecycle-fase **production** (sinds 2026-08-20, vóór de klantpilot) — migraties zijn additief en forward-only, een gelande migratie wordt niet meer herschreven; het `/v1/*`-contract blijft achterwaarts compatibel; destructieve operaties op prod vragen expliciete bevestiging. Solo-repo: feature-/fix-branch → tests groen → ff-merge naar `master`, geen PR-ceremonie. Open en forward-werk staat in GitHub-issues (`P*`/`epic/*`/`area/*`-labels; `/ai:next` rankt ze). Epics: `providers` (partner-koppelingen), `lookup` (non-Connection datalookup), `books`, `marketing` (publieke surface) en `platform` (livegang, infra, runtime). Entrypoints: `/ai:tdd`, `/ai:diagnose`, `/ai:to-issues`, `/ai:review`, `/ai:ship`. Detail in `docs/agents/workflow.md`.

Project-skills: **add-provider** (nieuwe partner-SDK bouwen + aan de Hub koppelen) · **change-sdk** (bestaande SDK wijzigen, met de tabel "raak ik de Hub aan?") · **unified-api-review** (architectuurreview van de Unified-API-kern door de provider-#2-lens; schrijft rapport, wijzigt geen code). Documentatie-drift: `/ai:docs-sync` (ai-kit, geen los project-skill), proactief na domein-wijzigingen en vóór commit. Animatie-taste (easing/duur/springs) via de emilkowalski-skills animation-tier — `animate`, `review-animations`, `improve-animations`, `find-animation-opportunities`, `animation-vocabulary` in `.claude/skills/` (gitignored; op een verse clone: `npx skills@latest add emilkowalski/skills -a claude-code -y -s animate review-animations improve-animations find-animation-opportunities animation-vocabulary`). Authoritative conventies staan in `.agents/rules/` (auto-loaded: taal, engineering, security); ai-kit canonical rules in `.claude/rules/` (gitignored, aanvullend).
