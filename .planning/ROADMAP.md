...
- **`PROV-MONEYBIRD`** — Nieuwe thin SDK `emeq/moneybird-api` (wrapt `picqer/moneybird-php-client ^0.42`, geen routes/Models/migrations — analoog Mollie-pattern D-12) + Hub-side `MoneybirdOAuthFlow` (registratie in `OAuthFlowRegistry`) + `config/hub-providers.php` row + Pennant kill-switch (auto-defined) + pass-through-API `/v1/moneybird/*` analoog Phase 5a + webhook-handler analoog Phase 5c. Repo `git@github.com:yusufkaracaburun/emeq-moneybird-api.git` is aangemaakt 2026-05-18 (publiek, leeg, geen branches yet — klaar voor v0.4-Phase-X SDK-foundation-commit). Bestaande `git@github.com:yusufkaracaburun/emeq-moneybird.git` (v1.2.0, self-contained Laravel-package met eigen OAuth/routes/Models/migrations sinds 2025-11-23) blijft leven voor non-Hub apps, wordt gedeprecated bij v0.4-release (README-banner naar `emeq/moneybird-api`). Trigger: na Naschool-live (Phase 14) en Snelstart-cert (Phase 12). Scope: ~10-15 plans gespreid over 3-4 phases. Beslissing-rationale: `project_moneybird_dual_shape_decision.md` (auto-memory).
