...
export default function PartnersIndex({ providers }: { providers: ProviderSummary[] }) {
    return (
        <ShowcaseLayout>
            <Head title="Integraties — Exact, Mollie, SnelStart" />
                    ...
                    {providers.map((provider, i) => (
                        <Reveal key={provider.key} delay={i * 0.06}>
                            <Link
                                ...
                                className="group flex h-full flex-col rounded-2xl border bg-card p-6 shadow-sm shadow-amber-500/5 transition-all hover:-translate-y-0.5 hover:border-amber-500/50 hover:shadow-xl hover:shadow-amber-500/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500 focus-visible:ring-offset-2 focus-visible:ring-offset-background"
                            >
