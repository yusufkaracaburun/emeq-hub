...
final class RelatedKeywordsController
{
    ...
    public function show(Request $request, DataForSeo $dataForSeo): JsonResponse
    {
        ...
        $validated = $request->validate([
            ...
            'depth' => ['sometimes', 'integer', 'min:0', 'max:4'],
        ...
        foreach (['location_code' => 'integer', 'language_code' => 'string', 'depth' => 'integer', 'limit' => 'integer'] as $field => $type) {
        ...
        try {
            $response = $this->pipeline->run(
                new PassThroughContext(
                    ...
                    method: 'GET',
                    path: '/dataforseo/related-keywords',
                    query: $validated,
                ),
                function () use ($dataForSeo, $keyword, $options): UpstreamResult {
                    $result = $dataForSeo->relatedKeywords($keyword, $options);
