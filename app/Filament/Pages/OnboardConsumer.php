...
class OnboardConsumer extends Page
{
    ...
    public function submit(): void
    {
        ...
        if ($webhookSecret === null || $webhookSecret === '') {
            ...
            $webhookSecretAutoGenerated = true;
        }
        ...
        $payload = [
            ...
            'connection' => self::buildConnectionPayload($data['connection'] ?? []),
            'token_name' => $data['pat']['token_name'],
            'abilities' => $abilities,
        ];

        try {
            $result = app(ConsumerOnboarding::class)->onboard($payload);
        } catch (\Throwable $e) {
            report($e);

            Notification::make()
                ->title('Onboarden mislukt')
                ->body('Er ging iets mis. Probeer opnieuw of bekijk Horizon-logs.')
                ->danger()
                ->send();
