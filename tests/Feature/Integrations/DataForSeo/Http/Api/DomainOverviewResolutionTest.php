...
final class DomainOverviewResolutionTest extends TestCase
{
    use RefreshDatabase;

    public function test_unauthenticated_request_returns_401(): void
    {
        $this->getJson('/v1/dataforseo/domain-overview?domain=example.com')
            ->assertStatus(401);
    }
    ...
    public function test_missing_domain_parameter_returns_422(): void
    {
        [$consumer, $token] = $this->consumerWithToken([TokenAbilities::DATAFORSEO_READ]);
        $account = Account::factory()->for($consumer)->create(['external_id' => 'school-A']);
        ...
        $this->withHeader('Authorization', "Bearer {$token}")
            ->withHeader('X-Account-Id', 'school-A')
            ->getJson('/v1/dataforseo/domain-overview')
            ->assertStatus(422)
            ->assertJsonPath('error', 'missing_domain');
    }
